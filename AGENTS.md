...
- **Vercel hosts both frontends**: web = `fleek3/iprs-web-v2` (Git-connected, auto-deploys on push to `main`, rootDirectory `apps/web`); console = `fleek3/iprs-dashboard` (CLI prebuilt flow **from repo root** — linking inside an app dir breaks uploads since build traces reference files above the app directory). NOTE 2026-09-14: the old `fleek3/iprs-web` project is gone (alias 404s); a same-named empty spare now exists without rootDirectory — either configure it (rootDirectory `apps/web`) or delete it in the Vercel dashboard to avoid duplicate failing auto-builds.
