...
export type VerificationInputType =
  'search' | 'number' | 'tel' | 'text' | 'none' | 'email' | 'url' | 'decimal' | 'file';
...
export const VERIFICATION_FORM_FIELDS: Record<VerificationType, VerificationFormField[]> = {
  iprs_standard: [
    {
      ...
      hint: 'Enter the 7–9 digit Kenyan ID number.',
      inputMode: 'numeric',
  ...
  match_id_phone: [
    ...
    },
    {
      key: 'phoneNumber',
      label: 'Phone number',
      hint: 'Use an 07 or +2547 Kenyan mobile number.',
      inputMode: 'tel',
  ...
  employer_verification: [
    ...
    },
    {
      key: 'employerName',
      label: 'Employer name',
      hint: 'Enter the employer exactly as registered.',
  ...
  face_id_match: [
    ...
    },
    {
      key: 'faceImageBase64',
      label: 'Face image',
      hint: 'Upload a clear JPEG or PNG face image (max 5 MB).',
      inputType: 'file',
      required: true,
    },
  ],
  bank_account_verification: [
    {
      ...
      hint: 'Enter the beneficiary account number.',
      inputMode: 'numeric',
    ...
    },
    {
      key: 'bankCode',
      label: 'Bank code',
      hint: 'Enter the bank code, for example 01 for KCB.',
      inputMode: 'numeric',
      placeholder: '01',
      required: true,
    },
    {
      key: 'idNumber',
      label: 'National ID number',
      hint: 'Enter the account holder ID when requested by the provider.',
      inputMode: 'numeric',
      placeholder: '12345678',
    },
  ],
  alien_id: [
    {
      ...
      hint: 'Enter the alien ID issued by Kenyan authorities.',
      placeholder: 'A123456',
      required: true,
    },
  ],
  ...
  passport_check: [
    ...
    },
    {
      key: 'nationality',
      label: 'Nationality',
      hint: 'Enter the nationality shown on the passport.',
      placeholder: 'Kenyan',
      required: true,
    },
  ],
  ...
  kplc_location_checker: [
    {
      key: 'meterNumber',
      label: 'KPLC meter number',
      hint: 'Enter the meter number shown on the electricity bill.',
      inputMode: 'numeric',
      placeholder: '12345678',
      required: true,
    },
  ...
  kra_pin_verification: [
    {
      key: 'kraPin',
      label: 'KRA PIN',
      hint: 'Enter a KRA PIN, for example A012345678Z.',
    ...
    },
    {
      key: 'idNumber',
      label: 'National ID number',
      hint: 'Optional when the KRA PIN is provided.',
      inputMode: 'numeric',
      placeholder: '12345678',
    },
  ],
  ...
  motor_vehicle_ownership: [
    {
      ...
      hint: 'Enter the registration number without spaces.',
      placeholder: 'KAA123A',
      required: true,
    },
  ],
  ...
  brs: [
    {
      ...
      hint: 'Enter the business registration number.',
      placeholder: 'BN123456',
      required: true,
    },
    {
      key: 'statementFileBase64',
      label: 'Business registration document',
      hint: 'Upload a PDF, JPEG, or PNG copy of the registration certificate (max 5 MB).',
      inputType: 'file',
      required: true,
    },
  ],
  ...
  scanned_statement: [
    ...
    },
    {
      ...
      hint: 'Upload a PDF, JPEG, or PNG statement (max 5 MB).',
      inputType: 'file',
      required: true,
    },
  ],
...
export function getVerificationFormValues(type: VerificationType): VerificationFormValues {
  return Object.fromEntries(
    VERIFICATION_FORM_FIELDS[type].map((field) => [field.key, '']),
  ) as VerificationFormValues;
}
...
export function isConsentGatingBlocked(
  values: VerificationFormValues,
  requiresCbConsent: boolean,
): boolean {
...
export function filterSensitiveResultEntries(
  result: Record<string, unknown> | null,
